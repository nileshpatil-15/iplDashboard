...
export default class TeamMatches extends Component {
    ...
    displayWholeTeamMatchDisplay = () => {
        ...
        return (
            <div className='inner-teammatch-container'>
              ...
              {recentMatches.map(eachCard=><MatchCard key={eachCard.id} data={eachCard}/>) }
    ...
    render() {
        const { isLoaderActive, teamMatchInfo } = this.state
        const { match } = this.props
        const { params } = match
        const { id } = params
        ...
        return (
            <div className={`team-match-container ${id}`}>
                ...
                {isLoaderActive ? <Loader data-testid='loader' type="Oval" color="#ffffff" height={50} />
